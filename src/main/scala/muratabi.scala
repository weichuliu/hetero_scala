...
class Node(val NID:Int, val layer:Int) {
	...
	val neiset:MSet[Node] = MSet() // neilist in python ver
	def dynamic_neighbours = Set() ++ (for (adjn <- adjcnt.keys; n <- adjn.adjcnt.keys) yield n)
	...
	def gen_neighbours() {
		neiset.clear()
		for (adjn <- adjcnt.keys) {
			neiset ++= adjn.adjcnt.keys
		}
		assert (neiset forall {_.layer == layer})
...
class Graph {
	...
	def MurataQ():Double = {
		...
		for (cl <- clist; c <- cl if !c.isEmpty) {
			elm += c.elm
			alam += c.degree * c.partner.get.degree
		}

		(elm.toDouble / M - alam.toDouble / (M * M)) / 2
	}
	...
	def calc_dQlist(layer:Int, nid:Int, neic_list:MSet[Community]) = {
		val node = nlist(layer)(nid)
		val src_c = node.comm
	...
	def all_pos_dQ(layer:Int, nid:Int) = {
		...
		val nc_list:MSet[Community] = node.neiset.map{_.comm}
